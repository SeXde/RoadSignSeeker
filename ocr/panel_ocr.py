...
class PanelTextOcr:
    ...
    def __init__(self, classifier: Classifier, dimension: DimReduction):
        self.line_detector = LineDetector()
        self.feature_extractor = FeatureExtractor()
        self.default_threshold = ImprovedThreshold((2, 2))
    ...
    def train_ocr(self, images_bgr, classes):
        ...
        for image_BGR, letter in zip(images_bgr, classes):
            ...
            contours, mask = self.default_threshold.threshold_image_class(image_gray)
            self.feature_extractor.extract(contours, ord(letter), mask, c, e)
    ...
    def classify_panel(self, image_bgr, panel_coordinates, image_bgr_name, output_file=None, show_image=False):
        ...
        contours, mask = self.default_threshold.threshold_image_class(panel_gray)
        ...
        for line in lines:
            for letter in line:
                ...
                self.feature_extractor.extract([letter], 0, mask, c, [])
